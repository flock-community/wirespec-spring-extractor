...
assert controller.contains("endpoint GetUser GET /users/{id"),
    "GetUser endpoint missing or wrong path:\n$controller"
assert controller.contains("endpoint CreateUser POST"),
...
assert types =~ /id\s*:\s*String\b(?!\?)/,
    "UserDto.id should be non-null String for Kotlin data class:\n$types"
assert types =~ /nickname\s*:\s*String\?/,
    "UserDto.nickname should be nullable String?:\n$types"
